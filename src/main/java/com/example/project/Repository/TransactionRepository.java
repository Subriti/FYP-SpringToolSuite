...
public interface TransactionRepository extends JpaRepository<Transaction,Integer> {
    ...
    @Query(value = "select * from transaction t join post p on p.post_id=t.post_id where t.reciever_user_id=?1 and t.post_id in "
    ...
    @Query(value = "select * from transaction t join post p on p.post_id= t.post_id where p.post_by=?1 and p.donation_status=3 "
